...
int PSPDialog::FinishShutdown() {
	if (ReadStatus() != SCE_UTILITY_STATUS_SHUTDOWN)
		return -1;
	INFO_LOG(Log::sceUtility, "sceUtilitySavedata StatusChanged: %i -> %i (within %i)", status, SCE_UTILITY_STATUS_NONE, 0);
...
void PSPDialog::ChangeStatusShutdown(int delayUs) {
	...
	INFO_LOG(Log::sceUtility, "sceUtilitySavedata StatusChanged: %i -> %i (within %i)", status, SCE_UTILITY_STATUS_SHUTDOWN, 0);
...
{
	if (force) {
		INFO_LOG(Log::sceUtility, "sceUtilitySavedata StatusChanged(Forced): %i -> %i (within %i)", status, SCE_UTILITY_STATUS_NONE, 0);
		ChangeStatus(SCE_UTILITY_STATUS_NONE, 0);
	} else {
		INFO_LOG(Log::sceUtility, "sceUtilitySavedata StatusChanged: %i -> %i (within %i)", status, SCE_UTILITY_STATUS_SHUTDOWN, 0);
		ChangeStatus(SCE_UTILITY_STATUS_SHUTDOWN, 0);
